...
from epos_2026 import get_inventory_path

inventory = dc.inventory(get_inventory_path())
```

> `dc.inventory` is new. It needs a DASCore newer than 0.1.20, which is why `pyproject.toml` installs the `dev` branch rather than a release. `02_the_inventory.py` is built on it.
